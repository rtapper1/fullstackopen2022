describe('Blog app', function () {
  ...
  describe('When logged in', function () {
    ...
    it('A blog can be deleted', function () {
      ...
      cy.get('.compact-blog').get('.full-info-button').click()
      cy.get('#main-view').should('contain', 'Stratechery')
      cy.get('.long-blog').get('.delete-button').click()
      cy.get('#main-view').should('not.contain', 'Stratechery')
    })
    ...
    it('A blog cannot be deleted by other user', function () {
      ...
      cy.get('.long-blog').get('.delete-button').should('not.exist')
    ...
    it('Blogs are in descending likes-order', function () {
      ...
      cy.contains('Exponential').contains('view').click()
      cy.get('.long-blog').contains('Exponential').contains('like').click()
      cy.get('.long-blog').contains('Exponential').contains('likes 1')

      // Exponential view should be first
      cy.get('.blog').eq(0).should('contain', 'Exponential view')

      // Expand Stratechery-blog and click like twice
      cy.contains('Stratechery').contains('view').click()
      cy.get('.long-blog').contains('Stratechery').contains('like').click()
      cy.get('.long-blog').contains('Stratechery').contains('likes 1')
      cy.get('.long-blog').contains('Stratechery').contains('like').click()
      cy.get('.long-blog').contains('Stratechery').contains('likes 2')

      // Stratechery should be first
      cy.get('.blog').eq(0).should('contain', 'Stratechery')
    })
  })
})
